...
#paquete para hacer queries sobre el dataset
library(sqldf)
...
switch ( Sys.info()[['sysname']],
         Windows = { directory.root  <-  "C:/Users/feder/OneDrive/Maestría Big Data/Laboratorio de Implementacion I/DM_EyF" },   #Windows
...
karch_dataset    <- "./datasets/datasets_paquete_premium_corregido_ext.csv.gz"
...
kmes_train_desde <- 202003  #Elijo entrenar desde que empezó la pandemia
...
SELECT W.*,
	CASE
					WHEN B3.CLASE IS NULL THEN W.CLASE_TERNARIA
					ELSE B3.CLASE
	END AS CLASE_TERNARIA_NUEVA
FROM
	(SELECT *
		FROM dataset
		WHERE 1 = 1
			AND FOTO_MES >= 202003
			AND FOTO_MES < 202012) W
LEFT JOIN
	(SELECT X.NUMERO_DE_CLIENTE,
			X.FOTO_MES,
			'BAJA+3' CLASE
		FROM dataset X
		WHERE 1 = 1
			AND X.FOTO_MES >= 202003
			AND X.FOTO_MES < 202011
			AND X.CLASE_TERNARIA = 'CONTINUA'
			AND NOT EXISTS
				(SELECT *
					FROM dataset Y
					WHERE 1 = 1
						AND Y.FOTO_MES >= 202003
						AND Y.FOTO_MES < 202011
						AND Y.FOTO_MES = X.FOTO_MES + 2
						AND Y.NUMERO_DE_CLIENTE = X.NUMERO_DE_CLIENTE ) ) B3 ON W.NUMERO_DE_CLIENTE = B3.NUMERO_DE_CLIENTE
AND W.FOTO_MES = B3.FOTO_MES
ORDER BY W.FOTO_MES,
	W.NUMERO_DE_CLIENTE
	")
...
campos_lags  <- setdiff(  colnames(dataset3) ,  c("clase_ternaria","clase01", "numero_de_cliente","foto_mes", campos_malos) )

#Hago feature Engineering en este mismo script
#agreglo los lags de orden 1
setorderv( dataset3, c("numero_de_cliente","foto_mes") )
dataset3[ , paste0( campos_lags, "_lag1") := shift(.SD, 1, NA, "lag"), 
...
{
   dataset3[,  paste0(vcol, "_delta1") := get( vcol)  - get(paste0( vcol, "_lag1"))]
}
...
if( kcanaritos > 0 )
{
  for( i  in 1:kcanaritos)  dataset3[ , paste0("canarito", i ) :=  runif( nrow(dataset3))]
}


#cargo los datos donde voy a aplicar el modelo
dapply  <- copy( dataset3[  foto_mes==kmes_apply ] )


#creo la clase_binaria2   1={ BAJA+2,BAJA+1}  0={CONTINUA}
dataset3[ , clase01:= ifelse( clase_ternaria=="CONTINUA", 0, 1 ) ]



#los campos que se van a utilizar
campos_buenos  <- setdiff( colnames(dataset3), c("clase_ternaria","clase01", campos_malos) )

#dejo los datos en el formato que necesita LightGBM
#uso el weight como un truco ESPANTOSO para saber la clase real
dtrain  <- lgb.Dataset( data= data.matrix(  dataset3[  foto_mes>=kmes_train_desde & foto_mes<=kmes_train_hasta , campos_buenos, with=FALSE]),
                        label=  dataset3[ foto_mes>=kmes_train_desde & foto_mes<=kmes_train_hasta, clase01],
                        weight=  dataset3[ foto_mes>=kmes_train_desde & foto_mes<=kmes_train_hasta , ifelse(clase_ternaria=="BAJA+2", 1.0000001, 1.0)] ,
                        free_raw_data= TRUE
                      )
